...
export const PurchaseDataTable: React.FC<PurchaseDataTableProps> = ({
    ...
    return (
        <div className="flex flex-col w-full gap-4">
            <Select
                isRequired
                 key={paymentMethod || "reset"}   // <- aquí la magia
                value={paymentMethod}   // valor controlado
...
//     onPaymentMethodChange
// }) => {
//     const { banksList, selectedBank, loading, error } = useSelector((state: RootState) => state.BanksAcounts);
//     const dispatch = useDispatch<AppDispatch>();
...
//             addToast({ title: "Texto copiado en el portapapeles" });
//         }).catch(err => {
...
//             >
//                 {banksList.map(bank => (
//                     <SelectItem key={bank.id} onClick={() => dispatch(selectBankById(bank.id))}>{bank.pay_method}</SelectItem>
//                 ))}
//             </Select>
